...
This script verifies that a Stage2PreferenceDataset batch can be connected to a
patched Stable Cascade Stage C prior with text conditioning and user
conditioning. It intentionally does not create a training loop, DPO loss, or
optimizer.
"""
...
def run_prior_forward_case(
    ...
    return ForwardCaseResult(
        ...
        has_nan=bool(torch.isnan(output_tensor).any().item()) if output_tensor.is_floating_point() else False,
    )
...
def _print_case_result(result: ForwardCaseResult, summary_only: bool) -> None:
    ...
    if not summary_only:
        print("status:", "failed: NaN detected" if result.has_nan else "ok")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Which image in the preference pair to encode as clip_img conditioning.",
    )
    parser.add_argument(
        "--skip-image-case",
        action="store_true",
        help="Skip the raw-image -> CLIP image encoder -> clip_img forward case.",
    )
    return parser
...
def main() -> int:
    ...
    try:
        # max_batches is present for CLI compatibility with future extensions.
        # This smoke test intentionally runs only one batch.
        _ = args.max_batches
        results.append(
            run_prior_forward_case(
                name="case A: text only",
                prior=prior,
                text=text,
                sample=sample,
                user_emb=None,
        ...
        results.append(
            run_prior_forward_case(
                name="case B: text + zero user embedding",
                prior=prior,
                text=text,
                sample=sample,
        ...
        results.append(
            run_prior_forward_case(
                name="case C: text + real user embedding",
                prior=prior,
                text=text,
                sample=sample,
                user_emb=user_emb,
        ...
        if clip_img is not None:
            results.append(
                run_prior_forward_case(
                    name=f"case D: text + {args.image_source} clip_img + real user embedding",
                    prior=prior,
                    text=text,
                    sample=sample,
                    user_emb=user_emb,
        ...
        passed = all(not result.has_nan for result in results)
    except Exception:
